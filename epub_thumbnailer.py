...
def extract_and_save(cover_path, cover_filename, epub):
    if cover_path:
        cover = epub.open(cover_path)
        im = Image.open(BytesIO(cover.read()))
        im = im.convert("RGB")
        # im.save(output_file)
        s3_utils.upload_PIL_image_cover(im, cover_filename, format='jpeg')
        return True
    return False

# input_file: path to input epub
# output_file: path to output JPG
def extract_cover(input_file, cover_filename):
    ...
    for strategy in extraction_strategies:
        try:
            ...
            if extract_and_save(cover_path, cover_filename, epub):
                return True
